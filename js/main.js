(function ($) {
    "use strict"; function portfolio_init() { var portfolio_grid = $('.portfolio-grid'), portfolio_filter = $('.portfolio-filters'); if (portfolio_grid) { portfolio_grid.shuffle({ speed: 450, itemSelector: 'figure' }); portfolio_filter.on("click", ".filter", function (e) { portfolio_grid.shuffle('update'); e.preventDefault(); $('.portfolio-filters .filter').parent().removeClass('active'); $(this).parent().addClass('active'); portfolio_grid.shuffle('shuffle', $(this).attr('data-group')); }); } }
...
$(".clients.owl-carousel").imagesLoaded().owlCarousel({ 
    ...
    nav: false,              // Kept false (no arrows)
    dots: false,             // Kept false (no dots)
    margin: 30,              // Increased from 10 to 30 for better spacing
    autoplay: true,          // NEW: Enables auto-sliding
    autoplayTimeout: 2000,   // NEW: Changes slide every 2 seconds
    autoplayHoverPause: true, // NEW: Pauses when mouse hovers
    autoplaySpeed: 800,      // NEW: Transition speed
    smartSpeed: 800,         // NEW: Speed of the slide
    responsive: { 
        0: { items: 2 }, 
        768: { items: 4 }, 
        1200: { items: 5 } 
